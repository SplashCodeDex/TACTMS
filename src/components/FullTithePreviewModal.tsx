      ...
      <>
        {parts.map((part, i) =>
          highlightWords.includes(part.toLowerCase()) ? (
            <mark key={i} className="bg-[var(--primary-accent-start)] text-[var(--text-on-accent)] px-0.5 rounded-sm">
              {part}
            </mark>
          ) : (
            part
...
interface TitheRecordRowProps {
    ...
    isSelected: boolean;
    navigateToNextAmountCellForEditing: (currentRecordId: number | string, direction: 'down' | 'up') => void;
...
const TitheRecordRow: React.FC<TitheRecordRowProps> = ({ 
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: record['No.'] });
  ...
  const renderCellContent = (field: keyof TitheRecordB, value: any) => {
    if (editingCell?.recordId === record['No.'] && editingCell?.field === field) {
      if (field === 'Transaction Date (\'DD-MMM-YYYY\')') {
        return (
          <input type="date" value={editedValue} onChange={(e) => setEditedValue(e.target.value)} onBlur={saveEdit}
            onKeyDown={(e) => { 
    ...
    return <span className="truncate block max-w-[180px]" title={textValue}><HighlightMatches text={textValue} highlight={searchTerm} /></span>;
  };

  const editableFields: (keyof TitheRecordB)[] = ['Membership Number', 'Transaction Date (\'DD-MMM-YYYY\')', 'Transaction Amount', 'Narration/Description'];

  return (
    <tr ref={setNodeRef} style={style} className={`${isSelected ? 'selected-row' : ''} ${isDragging ? 'shadow-lg bg-[var(--bg-card)]' : ''}`} id={`row-${record['No.']}`}>
      <td className="p-2 align-middle text-center">
        <button {...listeners} {...attributes} className="drag-handle p-1 text-[var(--text-muted)] hover:text-[var(--text-primary)] cursor-grab active:cursor-grabbing">
            <GripVertical size={16} />
        </button>
      </td>
      <td className="p-2 align-middle">
        <input type="checkbox" className="form-checkbox h-4 w-4 text-[var(--primary-accent-start)] border-[var(--border-color)] rounded focus:ring-[var(--primary-accent-start)] focus:ring-offset-[var(--bg-card)]"
          checked={isSelected} onChange={() => toggleRowSelection(record['No.'])} aria-label={`Select row ${visualIndex}`}/>
      </td>
...
interface FullTithePreviewModalProps {
  ...
  itemsPerPage: number;
  addToast: (message: string, type: ToastMessage['type'], duration?: number, actions?: ToastAction[]) => void;
  searchTerm: string;
  setSearchTerm: (term: string) => void;
  sortConfig: {key: keyof TitheRecordB, direction: 'asc'|'desc'} | null;
  setSortConfig: (config: {key: keyof TitheRecordB, direction: 'asc'|'desc'} | null) => void;
...
const FullTithePreviewModal: React.FC<FullTithePreviewModalProps> = (props) => {
  ...
  const [editingCell, setEditingCell] = useState<{ recordId: number | string; field: keyof TitheRecordB } | null>(null);
  ...
  const ALL_TABLE_COLUMNS = useMemo(() => {
    ...
    return (Object.keys(internalList[0]) as Array<keyof TitheRecordB>).filter(key => key !== 'No.');
  }, [internalList]);
  ...
  const filteredAndSortedTitheList = useMemo(() => {
    ...
    if (searchTerm) {
        const lowerSearchTerms = searchTerm.toLowerCase().split(' ').filter(Boolean);
        result = result.filter(record => {
            const searchableString = Object.values(record).join(' ').toLowerCase();
            return lowerSearchTerms.every(term => searchableString.includes(term));
        });
    }
    if (sortConfig) {
      ...
      result.sort((a, b) => {
        ...
        const numA = (valA === "" || valA == null) ? (direction === 'asc' ? Infinity : -Infinity) : Number(valA);
        const numB = (valB === "" || valB == null) ? (direction === 'asc' ? Infinity : -Infinity) : Number(valB);
  ...
  const handleTitheValueChangeInternal = useCallback((recordNo: number | string, field: keyof TitheRecordB, value: any) => {
    setInternalList(list => list.map(r => r['No.'] === recordNo ? { ...r, [field]: value } : r));
    setMadeChanges(true);
  }, []);
  ...
  const handleUndoDelete = useCallback(() => {
    ...
    setInternalList(list => {
        const listCopy = [...list];
        const sortedItemsToRestore = lastDeleted.items.sort((a, b) => b.index - a.index);
  ...
  const handleDeleteTitheRecordInternal = useCallback((recordNo: number | string) => {
    const deletedItemIndex = internalList.findIndex(r => r['No.'] === recordNo);
    if (deletedItemIndex === -1) return;
    
    const deletedItem = internalList[deletedItemIndex];
    setLastDeleted({ items: [{ record: deletedItem, index: deletedItemIndex }] });

    setInternalList(list => list.filter(r => r['No.'] !== recordNo));
    setSelectedRowIds(prev => { const newSet = new Set(prev); newSet.delete(recordNo); return newSet; });
    setMadeChanges(true);
    
    const undoAction: ToastAction = { label: 'Undo', onClick: handleUndoDelete, variant: 'primary' };
    addToast("Record deleted.", "info", Infinity, [undoAction]);
  }, [internalList, addToast, handleUndoDelete]);


  const startEditingInternal = useCallback((recordId: number | string, field: keyof TitheRecordB) => {
  ...
  const saveEditInternal = useCallback(() => {
    ...
    if (field === 'Transaction Amount') {
        ...
        try {
            ...
            const newDescription = recordToUpdate['Narration/Description'].includes('{DD-MMM-YYYY}')
                ? recordToUpdate['Narration/Description'].replace(/{DD-MMM-YYYY}/gi, newFormattedDate)
                : recordToUpdate['Narration/Description'];

            setInternalList(list => list.map(r => 
                r['No.'] === recordId ? { ...r, 'Transaction Date (\'DD-MMM-YYYY\')': newFormattedDate, 'Narration/Description': newDescription } : r
            ));
            setMadeChanges(true);

        } catch (e) {
            addToast(`Invalid date format for ${editedValue}. Use YYYY-MM-DD.`, 'error');
            return; // Keep editing
        }
    } else {
  ...
  }, [editingCell, editedValue, handleTitheValueChangeInternal, addToast, internalList]);
  ...
  const handleSmartMoveConfirm = useCallback(() => {
    ...
    if (isNaN(targetPos) || targetPos < 1 || targetPos > internalList.length + 1) {
        addToast(`Invalid target position.`, "error");
        setPendingSmartMove(null);
        return;
    }
    
    const oldIndex = internalList.findIndex(item => item['No.'] === pendingSmartMove['No.']);
    ...
    addToast(`Moved "${pendingSmartMove['Membership Number']}" to position ${targetPos}.`, "success");
  ...
  const toggleRowSelectionInternal = useCallback((recordNo: number | string) => {
    setSelectedRowIds(prev => { const newSet = new Set(prev); newSet.has(recordNo) ? newSet.delete(recordNo) : newSet.add(recordNo); return newSet; });
  }, []);
  ...
  const handleDeleteSelectedRowsInternal = useCallback(() => {
    if (selectedRowIds.size === 0) { addToast("No rows selected for deletion.", "warning"); return; }
    ...
    setInternalList(list => list.filter(r => !selectedRowIds.has(r['No.'])));
    setMadeChanges(true);
    
    const undoAction: ToastAction = { label: 'Undo', onClick: handleUndoDelete, variant: 'primary' };
    addToast(`${selectedRowIds.size} record(s) deleted.`, "info", Infinity, [undoAction]);
    setSelectedRowIds(new Set());
  ...
  useEffect(() => {
    if (selectAllCheckboxRef.current) {
      ...
      const numItemsSelected = sourceListForSelection.filter(item => selectedRowIds.has(item['No.'])).length;
      const totalItems = sourceListForSelection.length;
      selectAllCheckboxRef.current.indeterminate = totalItems > 0 && numItemsSelected > 0 && numItemsSelected < totalItems;
      selectAllCheckboxRef.current.checked = totalItems > 0 && numItemsSelected === totalItems;
    }
  }, [selectedRowIds, recordsToShow]);
  ...
  const modalTitle = madeChanges ? "Full Tithe List Preview & Edit *" : "Full Tithe List Preview & Edit";

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={modalTitle} size="xxl" 
      footerContent={
        <>
          <Button onClick={onClose} variant="outline" size="md">Close</Button>
          <Button onClick={handleApplyChangesWithCommit} variant="primary" size="md" disabled={!madeChanges} leftIcon={<Save size={16}/>}>Save &amp; Apply Changes</Button>
        </>
      }>
      ...
      <div className="space-y-4 relative">
        <div className="table-toolbar">
          <div className="actions-group">
               <Button onClick={openAddMemberToListModal} size="sm" variant="primary" leftIcon={<UserPlus size={16}/>}>Add Member</Button>
               <Button onClick={handleDeleteSelectedRowsInternal} size="sm" variant="danger" leftIcon={<Trash2 size={16}/>} disabled={selectedRowIds.size === 0}>Del Sel ({selectedRowIds.size})</Button>
          </div>
          <div className="flex items-center gap-2 flex-grow">
              <div className="actions-group relative">
                <Button ref={columnButtonRef} onClick={() => setIsColumnSelectorOpen(p => !p)} size="sm" variant="subtle" leftIcon={<Columns3 size={16}/>}>Columns</Button>
                <AnimatePresence>
                  {isColumnSelectorOpen && (
                      <motion.div ref={columnSelectorRef} initial={{ opacity: 0, scale: 0.95, y: -10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: -10 }}
                          ...
                          <h4 className="text-sm font-semibold mb-3 text-[var(--text-primary)]">Visible Columns</h4>
                          <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                              {ALL_TABLE_COLUMNS.map(key => (
                                  <label key={key} className="flex items-center space-x-3 cursor-pointer text-xs text-[var(--text-secondary)] hover:text-[var(--text-primary)]">
                                      <input type="checkbox" checked={columnVisibility[key] ?? true} onChange={() => setColumnVisibility(prev => ({...prev, [key]: !(prev[key] ?? true)}))}
                                      ...
                                      <span>{key.replace(" ('DD-MMM-YYYY')", "").replace("Membership Number", "Member Details")}</span>
                                  </label>
                              ))}
                          </div>
                      </motion.div>
              ...
              <div className="flex items-center gap-2">
                  <label htmlFor="target-pos-input" className="text-sm font-medium text-[var(--text-secondary)] whitespace-nowrap">Target Pos:</label>
                  <input id="target-pos-input" type="number" min="1" max={internalList.length || 1} value={targetPosition}
                      onChange={(e) => setTargetPosition(e.target.value)} className="form-input-light text-sm w-20 py-1.5" aria-label="Target position for reorder"/>
              </div>
              <Search size={20} className="text-[var(--text-muted)]"/>
              <input type="text" placeholder={`Search ${filteredAndSortedTitheList.length} records...`} value={searchInput}
                  onChange={(e) => setSearchInput(e.target.value)} 
                  onKeyDown={(e) => {
                      if (e.key === 'Enter' && pendingSmartMove) { e.preventDefault(); handleSmartMoveConfirm(); } 
                      else if (e.key === 'Escape') { setPendingSmartMove(null); }
                  }}
                  className="form-input-light text-sm w-full sm:w-auto sm:flex-grow py-1.5" aria-label="Search tithe list records"/>
          </div>
        </div>
        ...
        <AnimatePresence>
            {pendingSmartMove && (
                <motion.div initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}}
                    ...
                    Move <strong className="text-[var(--text-primary)]">{pendingSmartMove['Membership Number']}</strong> to position <strong className="text-[var(--text-primary)]">{targetPosition}</strong>? Press Enter to confirm.
                </motion.div>
            )}
        </AnimatePresence>

        <div className="overflow-y-auto border border-[var(--border-color)] rounded-lg shadow-md bg-[var(--bg-elevated)] max-h-[60vh]" ref={tableContainerRef}>
            <table className="min-w-full text-sm modern-table">
                <caption className="sr-only">Tithe list for editing. Can be sorted and reordered.</caption>
                <thead className="bg-[var(--bg-card-subtle-accent)] sticky top-0 z-10">
                    <tr>
                        <th scope="col" className="p-2 w-10" aria-label="Drag to reorder"></th>
                        <th scope="col" className="p-2 w-10">
                            <input type="checkbox" className="form-checkbox" ref={selectAllCheckboxRef} onChange={toggleSelectAllRowsInternal} aria-label="Select all rows" />
                        </th>
                        <th scope="col" className="p-2 w-12 text-center" onClick={() => requestSortFullPreviewInternal('No.')} style={{cursor: 'pointer'}}>
                            #
                        </th>
                        {visibleTableHeaders.map((key) => (
                ...
                <SortableContext items={filteredAndSortedTitheList.map(item => item['No.'])} strategy={verticalListSortingStrategy}>
                    <tbody>
                        ...
                        <TitheRecordRow
                            ...
                            navigateToNextAmountCellForEditing={navigateToNextAmountCellForEditing}
