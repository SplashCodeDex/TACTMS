...
const DataEntryRow = React.memo<DataEntryRowProps>(({ record, visualIndex, isActive, onAmountChange, onNavigate, setActiveRow }) => {
  const [localAmount, setLocalAmount] = useState(String(record['Transaction Amount'] ?? ''));
  ...
  return (
    ...
    >
      <td className="p-2 align-middle text-center text-xs text-[var(--text-muted)] w-12">{visualIndex}</td>
      <td className="p-2 align-middle text-sm text-[var(--text-primary)] w-2/3 truncate" title={record['Membership Number']}>
...
const DataEntryModal: React.FC<DataEntryModalProps> = ({ isOpen, onClose, onSave, titheListData }) => {
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    if (filterText) {
      data = data.filter(r => r['Membership Number'].toLowerCase().includes(filterText.toLowerCase()));
    }

    if (showOnlyEmpty) {
      data = data.filter(r => r['Transaction Amount'] === '' || r['Transaction Amount'] === null);
    }
  ...
  const handleAmountChange = useCallback((recordNo: number | string, amount: string) => {
    setLocalData(prevData =>
      prevData.map(record =>
        record['No.'] === recordNo ? { ...record, 'Transaction Amount': amount } : record
      )
    );
  }, []);

  const handleNavigation = useCallback((direction: 'up' | 'down') => {
    const currentIndex = filteredAndSortedData.findIndex(r => r['No.'] === activeRecordId);
  ...
  useEffect(() => {
    const rowElement = activeRecordId ? document.getElementById(`data-entry-row-${activeRecordId}`) : null;
    if (rowElement) {
  ...
  const { entriesFilled, totalEntries, progressPercentage } = useMemo(() => {
    ...
    const filled = localData.filter(r => {
        ...
        return !isNaN(amount) && r['Transaction Amount'] !== '' && r['Transaction Amount'] !== null;
    }).length;
    return {
    ...
    <Modal
      ...
      footerContent={
        <>
          ...
          <Button variant="primary" onClick={handleSave} leftIcon={<Check size={16}/>}>Done & Save Changes</Button>
        </>
      }
    >
        <div className="space-y-4">
            <div className="flex justify-between items-center flex-wrap gap-4 bg-[var(--bg-elevated)] p-3 rounded-lg border border-[var(--border-color)]">
                ...
                <div className="w-full sm:w-1/3">
                    <div className="flex justify-between text-xs text-[var(--text-secondary)] mb-1">
                        <span>Progress</span>
                        <span>{entriesFilled} / {totalEntries} Filled</span>
                    </div>
            ...
            <div className="overflow-y-auto border border-[var(--border-color)] rounded-lg shadow-md bg-[var(--bg-elevated)] max-h-[55vh]">
                <table className="min-w-full text-sm modern-table">
                    <caption className="sr-only">Table for rapid data entry of tithe amounts.</caption>
                    <thead className="bg-[var(--bg-card-subtle-accent)] sticky top-0 z-10">
                        <tr>
                            <th scope="col" className="p-2.5 text-center text-xs font-semibold uppercase tracking-wider w-12">No.</th>
                            <th scope="col" className="p-2.5 text-left text-xs font-semibold uppercase tracking-wider w-2/3">Member Details</th>
                            <th scope="col" className="p-2.5 text-left text-xs font-semibold uppercase tracking-wider w-1/3">Amount (GH₵)</th>
                        </tr>
                    </thead>
                    <tbody ref={tableBodyRef}>
                        {filteredAndSortedData.map((record, index) => (
                            <DataEntryRow
                                key={record['No.']}
                                record={record}
                                visualIndex={localData.findIndex(r => r['No.'] === record['No.']) + 1} // Show original index
