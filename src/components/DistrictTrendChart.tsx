...
const DistrictTrendChart: React.FC<DistrictTrendChartProps> = ({
  ...
  const titheTickValues = Array.from(
    { length: yAxisTitheTicks + 1 },
    (_, i) => Math.ceil(maxTithe / yAxisTitheTicks) * i,
  );

  // Y-Axis for Souls Won
  const yAxisSoulsTicks = Math.min(5, maxSouls + 1);
  const soulsTickValues =
    maxSouls > 0
      ? Array.from({ length: yAxisSoulsTicks + 1 }, (_, i) =>
          Math.round((maxSouls / yAxisSoulsTicks) * i),
        )
      : [0];
  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg
        ...
        aria-label="District performance trend chart"
      >
        <defs>
          <linearGradient id="titheAreaGradient" x1="0" y1="0" x2="0" y2="1">
            <stop
              offset="0%"
              stopColor="var(--primary-accent-start)"
              stopOpacity="0.4"
            />
            <stop
              offset="100%"
        ...
        <g transform={`translate(${margin.left}, ${margin.top})`}>
          ...
          {/* Tithe Path */}
          <MotionPath
            d={getAreaPath("totalTithe", maxTithe)}
            fill="url(#titheAreaGradient)"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.2 }}
          />
          <MotionPath
            d={getPath("totalTithe", maxTithe)}
            fill="none"
            stroke="var(--primary-accent-start)"
            strokeWidth="2"
          ...
          {/* Souls Won Path */}
          <MotionPath
            d={getAreaPath("soulsWon", maxSouls)}
            fill="url(#soulsAreaGradient)"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.5, delay: 0.4 }}
          />
          <MotionPath
            d={getPath("soulsWon", maxSouls)}
            fill="none"
            stroke="var(--success-start)"
            strokeWidth="2"
            initial={{ pathLength: 0 }}
            animate={{ pathLength: 1 }}
          ...
          <text
            transform={`translate(${-margin.left + 20}, ${chartHeight / 2}) rotate(-90)`}
            textAnchor="middle"
            className="fill-current text-[var(--text-primary)] text-sm"
          >
            Total Tithe (GH₵)
          </text>
          ...
          <text
            transform={`translate(${chartWidth + margin.right - 15}, ${chartHeight / 2}) rotate(90)`}
            textAnchor="middle"
            className="fill-current text-[var(--text-primary)] text-sm"
          >
            Souls Won
