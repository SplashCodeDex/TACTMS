  ...
  return (
    <div className="flex items-center p-4 rounded-xl shadow-lg transition-all duration-300 ease-in-out bg-[var(--bg-card-subtle-accent)] border border-[var(--border-color)] card-glow-on-hover">
      ...
      <div className="overflow-hidden">
        <p className="text-xs text-[var(--text-muted)] uppercase tracking-wider font-medium truncate" title={label}>{label}</p>
        <div className={`text-lg sm:text-xl font-bold text-[var(--text-primary)] truncate ${valueClassName || ''}`} title={typeof value === 'string' ? value : undefined}>
          {value}
        </div>
        {subValue && (
          <p className="text-xs text-[var(--text-secondary)] mt-0.5 truncate" title={subValue}>
            {subValue}
          </p>
        )}
      </div>
