...
// ============================================================================
// TESTS: Batch Reorder Logic
// ============================================================================

describe('memberOrderService.updateMemberOrder batch reorder', () => {
    /**
     * Apply batch updates to entries
     */
    function applyBatchUpdates(
        entries: Map<string, { titheBookIndex: number }>,
        updates: Array<{ memberId: string; newIndex: number }>,
        assemblyName: string
    ): Map<string, { titheBookIndex: number }> {
    ...
    it('updates multiple member positions', () => {
        const entries = new Map([
            ['central-tac001', { titheBookIndex: 1 }],
            ['central-tac002', { titheBookIndex: 2 }],
            ['central-tac003', { titheBookIndex: 3 }],
        ]);

        const updates = [
            { memberId: 'TAC001', newIndex: 3 },
            { memberId: 'TAC003', newIndex: 1 },
        ];

        const result = applyBatchUpdates(entries, updates, 'Central');

        expect(result.get('central-tac001')?.titheBookIndex).toBe(3);
        expect(result.get('central-tac003')?.titheBookIndex).toBe(1);
        expect(result.get('central-tac002')?.titheBookIndex).toBe(2); // Unchanged
    });

    it('ignores updates for non-existent members', () => {
        const entries = new Map([
            ['central-tac001', { titheBookIndex: 1 }],
        ]);

        const updates = [
            { memberId: 'TAC999', newIndex: 5 },
        ];

        const result = applyBatchUpdates(entries, updates, 'Central');

        expect(result.size).toBe(1);
        expect(result.get('central-tac001')?.titheBookIndex).toBe(1);
    });
});
...
// ============================================================================
// TESTS: Position Swap Logic
// ============================================================================

describe('memberOrderService.updateMemberPosition swap logic', () => {
    /**
     * Simulate position swap logic
     */
    function swapPositions(
        entries: Array<{ memberId: string; titheBookIndex: number }>,
        movingMemberId: string,
        newIndex: number
    ): Array<{ memberId: string; titheBookIndex: number }> {
        const result = entries.map(e => ({ ...e }));
        const movingEntry = result.find(e => e.memberId === movingMemberId);
        if (!movingEntry) return result;

        const oldIndex = movingEntry.titheBookIndex;
        const collision = result.find(e => e.titheBookIndex === newIndex && e.memberId !== movingMemberId);

        if (collision) {
            collision.titheBookIndex = oldIndex;
        }
        movingEntry.titheBookIndex = newIndex;

        return result;
    }

    it('swaps positions when collision occurs', () => {
        const entries = [
            { memberId: 'TAC001', titheBookIndex: 1 },
            { memberId: 'TAC002', titheBookIndex: 2 },
        ];

        const result = swapPositions(entries, 'TAC001', 2);

        expect(result.find(e => e.memberId === 'TAC001')?.titheBookIndex).toBe(2);
        expect(result.find(e => e.memberId === 'TAC002')?.titheBookIndex).toBe(1);
    });

    it('moves without swap when no collision', () => {
        const entries = [
            { memberId: 'TAC001', titheBookIndex: 1 },
            { memberId: 'TAC002', titheBookIndex: 2 },
        ];

        const result = swapPositions(entries, 'TAC001', 5);

        expect(result.find(e => e.memberId === 'TAC001')?.titheBookIndex).toBe(5);
        expect(result.find(e => e.memberId === 'TAC002')?.titheBookIndex).toBe(2);
    });

    it('handles non-existent member', () => {
        const entries = [
            { memberId: 'TAC001', titheBookIndex: 1 },
        ];

        const result = swapPositions(entries, 'TAC999', 5);
