...
const AgeFilterSection: React.FC<AgeFilterSectionProps> = React.memo(
  ({
    ...
    const disableInputs =
      originalData.length === 0 && !!uploadedFile && titheListData.length > 0;
      ...
      <div className="bg-[var(--bg-elevated)] p-6 rounded-xl border border-[var(--border-color)] h-full flex flex-col">
        <div className="flex-grow">
          <div aria-labelledby="age-filter-heading">
            ...
            {inputErrors.age && (
              <p id="ageError" className="form-error-text mt-2 col-span-2">
                {inputErrors.age}
              </p>
            )}
            {disableInputs && (
              <p className="text-xs text-[var(--text-muted)] mt-2">
                Age filter requires original member data. Re-upload Excel or
                load a favorite with member data to enable.
              </p>
            )}
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          <Button
            ...
            disabled={
              (!ageRangeMin && !ageRangeMax) || isLoading || disableInputs
            }
