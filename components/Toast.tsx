...
import { CheckCircle, XCircle, Info as InfoIcon, AlertTriangle, X } from 'lucide-react';
...
const toastColors: Record<ToastType, string> = {
    success: 'border-[var(--success-border)] bg-[var(--bg-card)] text-[var(--success-text)]',
    error: 'border-[var(--danger-border)] bg-[var(--bg-card)] text-[var(--danger-text)]',
    info: 'border-[var(--secondary-accent-end)] bg-[var(--bg-card)] text-[var(--text-primary)]',
    warning: 'border-[var(--warning-border)] bg-[var(--bg-card)] text-[var(--warning-text)]',
};

const Toast: React.FC<ToastProps> = ({ id, message, type, duration = 5000, actions, onDismiss }) => {
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="flex items-start space-x-3.5">
        ...
        <p className={`flex-grow text-sm font-medium min-w-0 break-words ${type === 'info' ? 'text-[var(--text-primary)]' : ''}`}>{message}</p>
        <button
      ...
      {actions && actions.length > 0 && (
        <div className="mt-2.5 pt-2.5 border-t border-[var(--border-color)]/30 flex gap-2.5 justify-end">
          {actions.map(action => (
            <Button 
              key={action.label} 
              onClick={() => { action.onClick(); handleManualDismiss(); }} 
              size="sm" 
