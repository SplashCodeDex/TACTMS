...
const MemberFilterPanel: React.FC<MemberFilterPanelProps> = ({
  ...
  const getFilteredData = useCallback(() => {
    ...
    if (nameSearch.trim()) {
      ...
      data = data.filter((m) => {
        const fullName =
          `${m["First Name"] || ""} ${m.Surname || ""} ${m["Other Names"] || ""}`.toLowerCase();
    ...
    if (isFilterActive) {
      ...
      data = data.filter((m) => {
        ...
        const hasAddress = !!(
          m["Postal Address"]?.trim() || m["Residential Address"]?.trim()
        );
  ...
  const handleExport = () => {
    ...
    try {
      exportToExcel(
        dataToExport,
        `FilteredMemberList-${new Date().toISOString().split("T")[0]}`,
      );
  ...
  const FilterSelect: React.FC<{
    ...
    onChange: (val: FilterCompleteness) => void;
  }> = ({ label, value, onChange }) => (
    <div>
      <label className="form-label text-xs">{label}</label>
      <select
        value={value}
        onChange={(e) => onChange(e.target.value as FilterCompleteness)}
        className="form-input-light text-sm w-full !py-1.5"
      >
    ...
    <div className="p-4 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-color)] space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        ...
        <FilterSelect
          ...
          onChange={(v) => handleFilterChange("whatsapp", v)}
        />
        <FilterSelect
          label="Address"
          value={filters.address}
          onChange={(v) => handleFilterChange("address", v)}
        />
      </div>
