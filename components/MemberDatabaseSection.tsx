...
const MemberDatabaseSection: React.FC<MemberDatabaseSectionProps> = ({
    ...
    if (selectedRows.size === filteredData.length && filteredData.length > 0) {
      ...
      setSelectedRows(
        new Set(
          filteredData
            .map((r) => r["No."])
            .filter((no) => no !== undefined) as (number | string)[],
        ),
      );
    }
  ...
  const handleCreateTitheList = () => {
    ...
    const selectedMembers = currentAssemblyData.data.filter(
      (m) => m["No."] !== undefined && selectedRows.has(m["No."]),
    );
      ...
      <section className="content-card">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {ASSEMBLIES.map((name) => (
            <button
              key={name}
              onClick={() => handleAssemblyClick(name)}
            ...
            >
              <p
                className={`font-bold ${activeAssembly === name ? "text-[var(--primary-accent-start)]" : "text-[var(--text-primary)]"}`}
              >
                {name} Assembly
              </p>
              <p className="text-sm text-[var(--text-secondary)]">
                {memberDatabase[name]?.data?.length || 0} members on record
              </p>
            </button>
          ))}
        </div>
      ...
      {activeAssembly && (
        <section className="content-card space-y-6">
          <h3 className="section-heading">
            Manage:{" "}
            <span className="text-gradient-primary ml-2">
          ...
          {!currentAssemblyData || currentAssemblyData.data.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-[var(--text-secondary)] mb-4">
                No master list found for this assembly.
              </p>
            ...
            <div>
              <div className="mb-6 p-4 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-color)]">
                <h4 className="text-md font-semibold text-[var(--text-primary)] mb-2 flex items-center gap-2">
                  <UploadCloud
                    size={18}
                ...
                <p className="text-sm text-[var(--text-secondary)] mb-4">
                  To update, upload a new file. This will replace the current{" "}
                  {currentAssemblyData.data.length} records for {activeAssembly}
                  .
                </p>
                <FileUploader onFileAccepted={handleUpload} />
              </div>
                ...
                <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                  <div className="flex items-center gap-4">
                    <h4 className="text-lg font-semibold text-[var(--text-primary)]">
                      Filtered Members ({filteredData.length})
                    </h4>
                    <Button
                  ...
                  <table className="min-w-full text-xs modern-table">
                    ...
                    <tbody>
                      {filteredData.map((member) => (
                        <tr
                          key={member["No."]}
                          className={`transition-colors duration-150 ${selectedRows.has(member["No."]!) ? "selected-row" : "hover:bg-[var(--bg-card-subtle-accent)]"}`}
                        >
                          <td className="p-2 align-middle">
                            <input
                              type="checkbox"
                              className="form-checkbox"
                              checked={selectedRows.has(member["No."]!)}
                              onChange={() =>
                                toggleRowSelection(member["No."]!)
                              }
                            />
                          </td>
                          <td className="p-2 align-middle text-[var(--text-primary)] font-medium truncate max-w-xs">{`${member["First Name"] || ""} ${member["Surname"] || ""}`}</td>
                          <td className="p-2 align-middle text-[var(--text-secondary)]">
                          ...
                          <td className="p-2 align-middle text-center">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                onEditMember(member, activeAssembly!);
                              }}
                            >
                              <Pencil size={14} />
                            </Button>
