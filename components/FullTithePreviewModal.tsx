      ...
      <td className="p-2 align-middle text-xs">
        <div className="flex items-center justify-center gap-1">
          <Button
            size="icon"
            variant="ghost"
            onClick={() => handleMoveRecord(record["No."])}
            className="!p-1.5"
...
const FullTithePreviewModal: React.FC<FullTithePreviewModalProps> = (props) => {
  ...
  const handleMoveRecord = useCallback(
    (recordNo: string | number) => {
      ...
      if (
        isNaN(targetPos) ||
        targetPos < 1 ||
        targetPos > internalList.length
      ) {
        addToast(
          `Invalid target position. Enter a number between 1 and ${internalList.length}.`,
          "error",
        );
        return;
      }
      ...
      addToast(
        `Moved "${movedMember["Membership Number"]}" to position ${targetPos}.`,
        "success",
      );
  ...
  const handleBatchUpdateAmountInternal = useCallback(() => {
    if (selectedRowIds.size === 0) {
      addToast("No rows selected to update amount.", "warning");
      return;
    }
    ...
    if (amountStr === "") {
      ...
      setMadeChanges(true);
      addToast(
        `Transaction amount cleared for ${selectedRowIds.size} selected record(s).`,
        "success",
      );
    } else {
      ...
      if (isNaN(amount) || amount < 0) {
        addToast(
          "Invalid amount. Please enter a non-negative number or leave empty.",
          "error",
        );
        return;
      }
      ...
      setMadeChanges(true);
      addToast(
        `Transaction amount set to ${amount.toFixed(2)} for ${selectedRowIds.size} selected record(s).`,
        "success",
      );
    }
    setBatchAmountInput("");
  ...
  const handleBatchUpdateDateInternal = useCallback(() => {
    if (selectedRowIds.size === 0) {
      addToast("No rows selected to update date.", "warning");
      return;
    }
    if (!batchDateInput) {
      addToast("Please select a date to apply.", "warning");
      return;
    }
    ...
    try {
      ...
      setInternalList((list) =>
        list.map((record) =>
          selectedRowIds.has(record["No."])
            ? { ...record, "Transaction Date ('DD-MMM-YYYY')": formattedDate }
            : record,
        ),
      );
      setMadeChanges(true);
      addToast(
        `Transaction date set to ${formattedDate} for ${selectedRowIds.size} record(s).`,
        "success",
      );
      setBatchDateInput("");
    } catch (e) {
              ...
              >
                <tbody>
                  {recordsToShow.length > 0 ? (
                    ...
                    <tr>
                      <td
                        colSpan={visibleTableHeaders.length + 4}
