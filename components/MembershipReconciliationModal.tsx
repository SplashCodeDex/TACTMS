...
import { MemberRecordA, MembershipReconciliationReport } from '../types';
import { Users, UserPlus, UserMinus, CheckSquare, Square, Inbox } from 'lucide-react';
...
const ReconciliationList: React.FC<ReconciliationListProps> = ({ title, icon, members, emptyText }) => (
    ...
    {members.length > 0 ? (
      <ul className="space-y-2 text-sm max-h-48 overflow-y-auto pr-2">
        {members.map((member, index) => {
            const name = `${member.Title || ''} ${member['First Name'] || ''} ${member.Surname || ''} ${member['Other Names'] || ''}`.replace(/\s+/g, ' ').trim() || 'Unnamed Member';
            const membershipId = member['Membership Number'] || member['Old Membership Number'] || 'N/A';
            return (
              <li key={member['No.'] || index} className="text-[var(--text-secondary)] p-2 rounded-md bg-[var(--bg-card)]">
...
const MembershipReconciliationModal: React.FC<MembershipReconciliationModalProps> = ({ isOpen, onClose, report, onKeepMembers }) => {
  ...
  const handleToggleSelectAll = () => {
      ...
      if (selectedMissingIds.size === report.missingMembers.length) {
          ...
          setSelectedMissingIds(new Set(report.missingMembers.map(m => m['No.']!).filter(Boolean)));
      }
  }

  const handleKeepSelected = () => {
    if (!report) return;
    const membersToKeep = report.missingMembers.filter(m => selectedMissingIds.has(m['No.']!));
  ...
  return (
    <Modal
      ...
      size="xl"
      footerContent={<Button onClick={onClose} variant="primary" size="md">Continue to Dashboard</Button>}
    >
      <div className="space-y-4">
        <div className="text-center">
            <Users size={40} className="mx-auto text-[var(--primary-accent-start)]" />
            <p className="mt-2 text-md text-[var(--text-secondary)]">
                Comparing current file with the <strong className="text-[var(--text-primary)]">{report.previousFileDate}</strong>.
            </p>
        </div>
            ...
            {report.missingMembers.length > 0 ? (
                <>
                    <ul className="space-y-1.5 text-sm max-h-48 overflow-y-auto pr-2">
                        ...
                        <li key={member['No.']} className="text-[var(--text-secondary)] p-2 rounded-md bg-[var(--bg-card)] flex items-center gap-3 hover:bg-[var(--border-color)]/30 transition-colors">
                            <button onClick={() => handleToggleSelection(member['No.']!)} className="flex-shrink-0 p-1">
                                {selectedMissingIds.has(member['No.']!) 
                                    ? <CheckSquare size={20} className="text-[var(--primary-accent-start)]" /> 
                                    : <Square size={20} className="text-[var(--text-muted)]" />
                                }
                            </button>
                            <div className="flex-grow overflow-hidden">
                                <p className="font-medium text-[var(--text-primary)] truncate">{`${member.Title || ''} ${member['First Name'] || ''} ${member.Surname || ''} ${member['Other Names'] || ''}`.replace(/\s+/g, ' ').trim() || 'Unnamed Member'}</p>
                                <p className="text-xs text-[var(--text-muted)]">ID: {member['Membership Number'] || member['Old Membership Number'] || 'N/A'}</p>
                            </div>
                        </li>
                        ))}
                    </ul>
                    <div className="mt-3 pt-3 border-t border-[var(--border-color)] flex items-center gap-3">
                        <Button variant="outline" size="sm" onClick={handleToggleSelectAll}>
                           {selectedMissingIds.size === report.missingMembers.length ? 'Deselect All' : 'Select All'}
                        </Button>
                        <Button 
                            variant="primary" 
                            size="sm" 
