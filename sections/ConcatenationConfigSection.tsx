...
import { DEFAULT_CONCAT_CONFIG } from '../constants';
...
const ConcatenationConfigSection: React.FC<ConcatenationConfigSectionProps> = React.memo(({
  concatenationConfig,
  ...
  return (
    <div className="bg-[var(--bg-elevated)] p-6 rounded-xl border border-[var(--border-color)] h-full">
        ...
        <div className="flex justify-between items-center">
            <h3 id="concat-config-heading" className="text-lg font-semibold flex items-center">
            <Layers size={20} className="mr-3 text-[var(--primary-accent-end)]" />
            ...
            text={
                ...
                <p className="text-xs">
                    This combined string identifies members. Numbers appear as (Main|Old) or (Number).
                </p>
                </>
            }
            />
        ...
        <div className="mt-4">
            ...
            {(Object.keys(DEFAULT_CONCAT_CONFIG) as Array<keyof ConcatenationConfig>).map((key) => (
                <CheckboxButton
                key={key}
