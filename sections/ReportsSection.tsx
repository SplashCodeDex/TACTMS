...
import { PieChart, DollarSign, Users, TrendingUp, LineChart } from 'lucide-react';
...
const ReportsSection: React.FC<ReportsSectionProps> = ({ transactionLog, memberDatabase }) => {
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());

  const yearOptions = useMemo(() => {
    const years = new Set(transactionLog.map(f => new Date(f.timestamp).getFullYear()));
      ...
      monthlyData[month] = ASSEMBLIES.map(name => {
          ...
          const soulsWonForMonth = assemblyMasterList.filter(member => {
              ...
              try {
                  ...
                  return seenDate.getMonth() === month && seenDate.getFullYear() === selectedYear;
              } catch(e) { return false; }
          }).length;

          const assemblyLogsForMonth = transactionLog.filter(log => {
              const logDate = new Date(log.selectedDate);
              return log.assemblyName === name && logDate.getMonth() === month && logDate.getFullYear() === selectedYear;
          });

          const monthlyTotals = assemblyLogsForMonth.reduce((acc, log) => {
  ...
  const yearSummary = useMemo(() => {
    const summary = {
        ...
        monthlyPerformance: Array(12).fill(0).map((_, i) => ({ month: i, totalTithe: 0, soulsWon: 0 })),
    };

    const assemblyTotals = new Map<string, { totalTithe: number, soulsWon: number }>();
    ...
    for(let month = 0; month < 12; month++) {
        ...
        reportDataByMonth[month].forEach(report => {
            ...
            const current = assemblyTotals.get(report.assemblyName) || { totalTithe: 0, soulsWon: 0 };
        ...
        summary.monthlyPerformance[month] = { month, totalTithe: monthTithe, soulsWon: monthSouls };
    }
    
    const sortedByTithe = [...assemblyTotals.entries()].sort((a, b) => b[1].totalTithe - a[1].totalTithe);
    if(sortedByTithe.length > 0 && sortedByTithe[0][1].totalTithe > 0) {
        summary.topPerformingAssembly = { name: sortedByTithe[0][0], value: sortedByTithe[0][1].totalTithe };
    }
    
    const sortedByGrowth = [...assemblyTotals.entries()].sort((a, b) => b[1].soulsWon - a[1].soulsWon);
    if(sortedByGrowth.length > 0 && sortedByGrowth[0][1].soulsWon > 0) {
        summary.topGrowthAssembly = { name: sortedByGrowth[0][0], value: sortedByGrowth[0][1].soulsWon };
    }

    return summary;
  }, [reportDataByMonth]);
  
  const hasDataForPeriod = yearSummary.totalTithe > 0 || yearSummary.totalSouls > 0;
    ...
    <div className="space-y-8">
      <section className="content-card" aria-labelledby="reports-heading">
        <div className="flex justify-between items-center flex-wrap gap-4">
            <h2 id="reports-heading" className="section-heading border-none pb-0 mb-0">
                <PieChart size={22} className="mr-3 icon-primary" />
                Annual Performance Dashboard
            </h2>
            <div className="flex items-center gap-4">
                <select value={selectedYear} onChange={e => setSelectedYear(Number(e.target.value))} className="form-input-light">
      ...
      {hasDataForPeriod ? (
        ...
        <motion.div className="space-y-8" initial="hidden" animate="visible" variants={containerVariants}>
            <motion.section variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatDisplayCard 
                    icon={<DollarSign />} label={`District Total Tithe (${selectedYear})`}
                    value={<>GH₵ <AnimatedNumber n={yearSummary.totalTithe} formatter={(n) => n.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}/></>} theme="dark"
                />
                 <StatDisplayCard 
                    icon={<Users />} label={`District Souls Won (${selectedYear})`}
                    value={<AnimatedNumber n={yearSummary.totalSouls} />} theme="dark"
                />
                 <StatDisplayCard 
                    icon={<TrendingUp />} label="Top Assembly (Tithe)"
                    value={yearSummary.topPerformingAssembly.name}
                    subValue={`GH₵ ${yearSummary.topPerformingAssembly.value.toLocaleString()}`}
                    valueClassName="text-gradient-primary" theme="dark"
                />
                 <StatDisplayCard 
                    icon={<TrendingUp />} label="Top Assembly (Growth)"
             ...
             <motion.section variants={itemVariants} className="content-card">
                ...
                <DistrictTrendChart performanceData={yearSummary.monthlyPerformance} />
            </motion.section>
        ...
        <div className="text-center py-12 content-card">
            <PieChart size={52} className="mx-auto text-[var(--text-muted)] mb-4 opacity-50" />
            <p className="text-[var(--text-secondary)] text-lg">No report data for this period.</p>
            <p className="text-[var(--text-muted)] text-sm mt-1">
              Download a processed list from the "Tithe Processor" view to populate reports.
            </p>
        </div>
      )}
    </div>
