    ...
    return (
        ...
        >
            <motion.div variants={itemVariants}>
                <h1 className="text-3xl font-bold text-[var(--text-primary)]">{greeting}{userProfile ? `, ${userProfile.name.split(' ')[0]}` : ''}.</h1>
                <p className="text-lg text-[var(--text-secondary)]">Here's your district's overview for today.</p>
            </motion.div>

            <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <StatDisplayCard 
                    icon={<DollarSign />} label={`Year-to-Date Tithe (${new Date().getFullYear()})`}
                    value={<>GH₵ <AnimatedNumber n={stats.ytdTithe} formatter={(n) => n.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}/></>}
                    theme={theme}
                />
                <StatDisplayCard 
                    icon={<TrendingUp />} label={`Year-to-Date Souls Won`}
                    value={<AnimatedNumber n={stats.ytdSouls} />}
                    theme={theme}
                />
                <StatDisplayCard 
                    icon={<Users />} label="Total Members on Record"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <motion.section variants={itemVariants} className="lg:col-span-2 content-card space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        <div className="p-6 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-color)] text-left space-y-4">
                            <h3 className="font-semibold text-[var(--text-primary)]">Start New Weekly List</h3>
                            <p className="text-sm text-[var(--text-secondary)]">Load the latest tithe list for an assembly.</p>
                            <div className="flex items-end gap-3">
                                <div className="flex-grow">
                                    <label htmlFor="assembly-start-select-dash" className="form-label">Select Assembly</label>
                                    <select
                                    ...
                                    >
                                        ...
                                        {ASSEMBLIES.map(assembly => (
                                            <option key={assembly} value={assembly} disabled={!assembliesWithFavorites.has(assembly)}>
                                                {assembly} {assembliesWithFavorites.has(assembly) ? '' : '(No saved data)'}
                                            </option>
                                        ))}
                                    </select>
                                </div>
                                <Button onClick={handleStartWeek} disabled={!selectedAssembly} leftIcon={<Building2 size={16}/>}>Start</Button>
                            </div>
                        </div>
                         <div className="p-6 bg-[var(--bg-elevated)] rounded-xl border border-[var(--border-color)] text-left space-y-4 flex flex-col justify-center">
                             <h3 className="font-semibold text-[var(--text-primary)]">Upload a New List File</h3>
                             <p className="text-sm text-[var(--text-secondary)]">If you have a new file to process from an assembly.</p>
                             <Button onClick={handleUploadClick} fullWidth variant="secondary" size="lg" leftIcon={<UploadCloud size={18}/>}>
                                Upload Excel File
                             </Button>
                    ...
                    {stats.recentActivities.length > 0 ? (
                        <ul className="space-y-3">
                            {stats.recentActivities.map(log => (
                                <li key={log.id + log.timestamp} className="flex items-center gap-4 p-3 rounded-lg hover:bg-[var(--bg-card-subtle-accent)] transition-colors">
                                    ...
                                    <div className="overflow-hidden">
                                        <p className="font-semibold text-sm text-[var(--text-primary)] truncate">{log.assemblyName} Assembly</p>
                                        <p className="text-xs text-[var(--text-secondary)]">
                                            {formatDateDDMMMYYYY(new Date(log.selectedDate))}
                                            <span className="text-[var(--text-muted)] mx-1">•</span>
                                            <span className="font-medium text-[var(--success-text)]">GH₵ {log.totalTitheAmount.toLocaleString()}</span>
                                        </p>
                                    </div>
                                </li>
                            ))}
                        ...
                        <p className="text-sm text-center py-8 text-[var(--text-muted)]">No recent transactions logged.</p>
                    )}
                </motion.section>
            </div>
        </motion.div>
