...
const OutreachSkeleton: React.FC = () => (
    <div className="space-y-4">
      {[...Array(3)].map((_, i) => (
        <div key={i} className="p-4 bg-[var(--bg-card-subtle-accent)] rounded-lg animate-pulse">
...
`;
                    ...
                    responseSchema: {
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                message: {
                                    type: Type.STRING,
                                    description: "The personalized welcome SMS message for the member."
                                }
                            },
            ...
            setError("Sorry, I couldn't generate the messages. The AI might be busy. Please try again in a moment.");
    ...
    return (
        <section className="content-card">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                <div>
                    <label htmlFor="outreach-target" className="form-label">Select Target Group</label>
                ...
                {!isLoading && !error && messages.length > 0 && (
                    <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                        {messages.map((msg, index) => (
                            <div key={index} className="p-4 bg-[var(--bg-elevated)] rounded-lg border border-[var(--border-color)]">
                                <div className="flex justify-between items-center mb-2">
                                    <h4 className="font-semibold text-[var(--text-primary)] flex items-center gap-2">
                                        <Users size={16}/> {msg.memberName}
                                    </h4>
                                    <Button size="sm" variant="ghost" leftIcon={<Copy size={14}/>} onClick={() => handleCopy(msg.message)}>
                                        Copy
                                    </Button>
                                </div>
                 ...
                 {!isLoading && !error && messages.length === 0 && (
                    <div className="text-center py-6 text-[var(--text-muted)]">
                       {newMembers.length > 0 ? "Click 'Generate' to create personalized welcome messages for your new members." : "No new members found in the current workspace. Upload and reconcile a new file to identify them."}
                    </div>
                 )}
            </div>
        </section>
